...
def test_make_peel_geodesic_dogbone():
    ...
    peel, _ = utilFunc.make_peel_geodesics(leaf_locs)

    assert np.allclose(peel, np.array([[0, 1, 4],
                                       [3, 2, 5],
                                       [4, 5, 6]]))


def test_peel_geodesic_example():
    # Example
    # Note that the LCA of node 0 and 4 is on the geodesic extended beyond node 0

    leaf_locs = torch.tensor([[1.5330e-02, 1.0385e-02],
                             [-6.0270e-02, 4.4329e-02],
                             [-6.1253e-02, -1.5676e-01],

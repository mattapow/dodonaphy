...
import pytest
import torch
from src import peeler, poincare, utils, Cutils
...
def test_make_peel_example1():
    ...
    int_locs = torch.tensor([[0.0352, 0.0405], [0.0437, 0.0144],
                             [0.0375, -0.070], [-0.0633, -0.1595]],
                            dtype=torch.float64)
...
def test_make_peel_incentre():
    leaf_locs = torch.tensor([[0.06644704, 0.18495312, -0.03839615],
...
def test_make_peel_geodesic_dogbone():
    leaf_r = torch.tensor([.5, .5, .5, .5])
    leaf_theta = torch.tensor(
        [np.pi / 10, -np.pi / 10, np.pi * 6 / 8, -np.pi * 6 / 8])
    leaf_dir = utils.angle_to_directional(leaf_theta)
...
def test_make_peel_geodesic_example0():
    leaf_locs = torch.tensor([(0.05909264, 0.16842421, -0.03628194),
...
def test_compute_Q():
    ...
    Q1 = torch.tensor([[0, -50, -38, -34, -34], [-50, 0, -38, -34, -34],
                       [-38, -38, 0, -40, -40], [-34, -34, -40, 0, -48],
                       [-34, -34, -40, -48, 0]]).double()
...
def test_soft_nj_knownQ():
    ...
    assert sum(
        peel_check), "Wrong topology. NB. not all correct cases covered."
    assert torch.isclose(
        sum(blens).double(),
        torch.tensor(17).double()), "Wrong sum of branch lengths."

...
% Q-criterion, which is used to determine which edges are joined during neighbour joining (NJ): 
%\be
...
%\be
%w_{ij} = \frac{\exp(-1/Q_{ij})}{\sum_{j} \exp(-1/Q_{ij})}.
%\ee
%Nodes that are closer neighbours according to the Q-criterion have less weight (more emphasis).

%The additional scaling by the current epoch number $n$ is put in as a cooling effect so that:
%\be
%\lim_{n\to\infty} w_{ij} = 
%\begin{cases}
%1 \qquad if \min(Q_{ij})\\
%0 \qquad otherwise
%\end{cases}
%\ee
